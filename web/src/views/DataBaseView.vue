<template>
  <div class="database-container layout-container">
    <PageHeader
      ...
      :show-border="true"
      aria-label="지식베이스 보기 전환"
    />

    <PageShoulder v-model:search="searchQuery" search-placeholder="지식베이스 검색...">
      <template #filters>
        <a-select
          ...
          allow-clear
        >
          <a-select-option :value="null">모든 유형</a-select-option>
    ...
    >
      <div class="new-database-form">
        ...
        <div class="form-section">
          <h3 class="section-title">지식베이스 유형<span class="required-mark">*</span></h3>
        ...
        <div class="form-section">
          <h3 class="section-title">지식베이스 이름<span class="required-mark">*</span></h3>
          <a-input v-model:value="newDatabase.name" placeholder="지식베이스 이름" />
        </div>

        <div v-if="selectedKbTypeInfo?.requires_embedding_model" class="form-grid two-columns">
          <div class="form-section compact-section">
            <h3 class="section-title">임베딩 모델</h3>
            <EmbeddingModelSelector
              v-model:value="newDatabase.embedding_model_spec"
              class="full-width"
              placeholder="임베딩 모델을 선택하세요"
            />
          </div>

          <div class="form-section compact-section">
            <div class="chunk-preset-title-row">
              <h3 class="section-title">청크 분할 전략</h3>
        ...
        <div class="form-section">
          <h3 class="section-title">지식베이스 설명</h3>
          <p class="field-hint description-hint">
            이 설명은 에이전트가 도구를 고를 때 사용됩니다. 이름과 설명을 구체적으로 작성할수록 적절한 지식베이스를 더 잘 선택합니다.
          </p>
          <AiTextarea
        ...
        <!-- 共享配置 -->
        <div class="form-section compact-section">
          <h3 class="section-title">공유 설정</h3>
          <ShareConfigForm
      ...
      <template #footer>
        <a-button key="back" @click="cancelCreateDatabase">취소</a-button>
        <a-button
          key="submit"
          type="primary"
    ...
    <ResourceEmptyState
      ...
      title="지식베이스가 없습니다"
      description="지식베이스를 만든 뒤 파일을 업로드하고 검색, 그래프, 평가 기능을 설정할 수 있습니다."
    ...
    <ExtensionCardGrid v-else>
      <InfoCard
        ...
        :description="database.description || '설명 없음'"
...
const knowledgeViewItems = [
  { key: 'documents', label: '문서 지식베이스', path: '/extensions?tab=knowledge' }
]
...
const loadSupportedKbTypes = async () => {
  try {
    ...
    newDatabase.kb_type = ''
    resetCreateParamValues()
    message.error('지식베이스 유형을 불러오지 못했습니다. 잠시 후 다시 시도하세요')
  }
}
...
const formatCreatedTime = (createdAt) => {
  ...
  if (diffInDays < 7) {
    return `${diffInDays}일 전 생성됨`
  }
  if (diffInDays < 30) {
    const weeks = Math.floor(diffInDays / 7)
    return `${weeks}주 전 생성됨`
  }
  if (diffInDays < 365) {
    const months = Math.floor(diffInDays / 30)
    return `${months}개월 전 생성됨`
  }
  const years = Math.floor(diffInDays / 365)
  return `${years}년 전 생성됨`
}
...
const handleCreateDatabase = async () => {
  if (!selectedKbTypeInfo.value) {
    message.error('지식베이스 유형을 불러오지 못해 만들 수 없습니다')
    return
  }
  ...
  for (const field of createParamOptions.value) {
    ...
    if (value === undefined || value === null || (typeof value === 'string' && !value.trim())) {
      message.error(`${field.label || field.key} 항목을 입력하세요`)
      return
    }
  }
...
const cardSubtitle = (database) => {
  ...
  if (!kbUtils.isReadOnlyDatabase(database)) {
    parts.push(`파일 ${database.row_count || 0}개`)
  }
  return parts.join(' · ')
}
...
const copyDatabaseId = async (database) => {
  ...
  message.success('지식베이스 ID를 복사했습니다')
}

const deleteDatabase = (database) => {
  Modal.confirm({
    title: '지식베이스 삭제',
    content: `지식베이스 “${database.name}”을(를) 삭제할까요? 이 작업은 되돌릴 수 없습니다.`,
    okText: '삭제',
    okType: 'danger',
    cancelText: '취소',
    onOk: async () => {
