  ...
  <div class="login-view" :class="{ 'has-alert': serverStatus === 'error' }">
    ...
    <div v-if="serverStatus === 'error'" class="server-status-alert">
      <div class="alert-content">
        ...
        <div class="alert-text">
          <div class="alert-title">서버에 연결할 수 없습니다</div>
    ...
    <main class="login-main">
      <div class="login-card">
        ...
        <div class="card-side is-image">
          <img :src="loginBgImage" alt="로그인 배경" class="login-bg-image" />
        </div>
        ...
        <div class="card-side is-form">
          <div class="form-wrapper">
            <header class="form-header">
              <!-- 如果是在初始化，显示特定标题 -->
              <h2 v-if="isFirstRun" class="init-title">처음 시작합니다. 최고 관리자 계정을 만드세요.</h2>
              <p v-else class="welcome-text">다시 오셨네요</p>
            </header>
                ...
                <a-form :model="adminForm" @finish="handleInitialize" layout="vertical">
                  <a-form-item
                    ...
                    :rules="[
                      ...
                      {
                        pattern: /^[a-zA-Z0-9_]+$/,
                        message: '사용자 ID에는 영문, 숫자, 밑줄(_)만 사용할 수 있습니다'
                      },
                      {
                        min: 3,
                        max: 20,
                        message: '사용자 ID는 3~20자로 입력하세요'
                      }
                    ]"
                  >
                    <a-input
                      v-model:value="adminForm.uid"
                      placeholder="사용자 ID (3~20자)"
                      :maxlength="20"
                    />
                  </a-form-item>
                    ...
                    :rules="[
                      {
                        validator: async (rule, value) => {
                          ...
                          if (!phoneRegex.test(value)) {
                            throw new Error('올바른 휴대폰 번호 형식으로 입력하세요')
                          }
                        }
                      }
                    ]"
              ...
              <div v-else class="login-form">
                <a-form :model="loginForm" @finish="handleLogin" layout="vertical">
                  <a-form-item
                    label="로그인 ID"
                    name="loginId"
                    :rules="[{ required: true, message: '사용자 ID 또는 휴대폰 번호를 입력하세요' }]"
                  >
                    <a-input v-model:value="loginForm.loginId" placeholder="사용자 ID 또는 휴대폰 번호">
                      <template #prefix>
                        <user-icon size="18" />
                      </template>
                    </a-input>
                  ...
                  <a-form-item>
                    <a-button
                      ...
                      size="large"
                    >
                      <span v-if="isLocked">계정이 잠겼습니다 · {{ formatTime(lockRemainingTime) }}</span>
    ...
    <footer class="page-footer">
      <div class="footer-links">
        <a href="https://github.com/pyhgoshift/AI-KnowledgeHub/issues" target="_blank">문의하기</a>
        <span class="divider">|</span>
        <a href="https://github.com/pyhgoshift/AI-KnowledgeHub" target="_blank">도움말</a>
      </div>
      <div class="copyright">
        &copy; {{ new Date().getFullYear() }} {{ brandName }}. 모든 권리 보유.
      </div>
    </footer>
  </div>
</template>
...
const oidcButtonText = ref('OIDC 로그인')
...
const validateConfirmPassword = async (rule, value) => {
  if (value === '') {
    throw new Error('비밀번호를 한 번 더 입력하세요')
  }
...
const ensureAgreementAccepted = () => {
  ...
  const warningMessage = '이용약관과 개인정보 처리방침에 먼저 동의하세요'
...
const handleLogin = async () => {
  // 如果当前被锁定，不允许登录
  if (isLocked.value) {
    message.warning(`계정이 잠겼습니다. ${formatTime(lockRemainingTime.value)} 후 다시 시도하세요`)
    return
  }
  ...
  try {
    ...
    if (error.status === 423) {
      ...
      if (remainingTime === 0) {
        const lockTimeMatch = error.message.match(/(\d+)\s*(?:초|秒)/)
        if (lockTimeMatch) {
      ...
      if (remainingTime > 0) {
        ...
        errorMessage.value = `로그인 실패가 반복되어 계정이 ${formatTime(remainingTime)} 동안 잠겼습니다`
      } else {
        errorMessage.value = error.message || '계정이 잠겼습니다. 잠시 후 다시 시도하세요'
      }
    } else {
      errorMessage.value = error.message || '로그인하지 못했습니다. 사용자 ID와 비밀번호를 확인하세요'
    }
  } finally {
    loading.value = false
  }
...
const handleOIDCLogin = async () => {
  ...
  try {
    ...
    if (response.login_url) {
      ...
      errorMessage.value = 'OIDC 로그인 주소를 가져오지 못했습니다'
    }
  } catch (error) {
    console.error('OIDC 登录失败:', error)
    errorMessage.value = error.message || 'OIDC 로그인에 실패했습니다. 다시 시도하세요'
  } finally {
...
const handleInitialize = async () => {
  ...
  try {
    ...
    if (adminForm.password !== adminForm.confirmPassword) {
      errorMessage.value = '입력한 비밀번호가 서로 다릅니다'
      return
    }
    ...
    message.success('관리자 계정을 만들었습니다')
    router.push('/')
  } catch (error) {
    console.error('初始化失败:', error)
    errorMessage.value = error.message || '초기 설정에 실패했습니다. 다시 시도하세요'
  } finally {
    loading.value = false
  }
}
...
const checkFirstRunStatus = async () => {
  try {
    ...
    errorMessage.value = '시스템 오류가 발생했습니다. 잠시 후 다시 시도하세요'
  } finally {
    loading.value = false
  }
}
...
const checkServerHealth = async () => {
  try {
    ...
    if (response.status === 'ok') {
      ...
      serverError.value = response.message || '서버 상태가 정상적이지 않습니다'
    }
  } catch (error) {
    console.error('检查服务器健康状态失败:', error)
    serverStatus.value = 'error'
    serverError.value = error.message || '서버에 연결할 수 없습니다. 네트워크 연결을 확인하세요'
  } finally {
