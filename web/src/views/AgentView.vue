              ...
              <template #overlay>
                <div class="config-dropdown-panel" @click.stop>
                  ...
                  >
                    ...
                    <span v-if="agent.isBuiltin" class="config-dropdown-item-badge">기본 제공</span>
                    <Check
                  ...
                  <div v-if="hasActiveThread" class="config-dropdown-hint">
                    현재 대화에는 에이전트가 연결되어 있습니다. 새 대화에서 변경할 수 있습니다.
                  </div>
                  ...
                  >
                    ...
                    <span class="config-dropdown-item-label">에이전트 관리</span>
                  </button>
                </div>
              </template>
            </a-dropdown>
...
const currentAgentLabel = computed(() => {
  if (isLoadingConfig.value) return '불러오는 중...'
  return currentAgentOption.value?.label || '에이전트'
})
...
const handleAgentSwitch = async (agentId, hasActiveThread) => {
  ...
  if (hasActiveThread) {
    message.info('현재 대화에 에이전트가 연결되어 있습니다. 새 대화에서 변경하세요')
    return
  }
  try {
    ...
    console.error('에이전트 변경 오류:', error)
    message.error('에이전트 변경에 실패했습니다')
  }
}
...
const openAgentManagement = async () => {
  agentDropdownOpen.value = false
  if (!selectedAgentId.value) {
    message.warning('먼저 에이전트를 선택하세요')
    return
  }
  try {
    ...
    message.error(error.message || '에이전트 설정을 열지 못했습니다')
  }
}
</script>
