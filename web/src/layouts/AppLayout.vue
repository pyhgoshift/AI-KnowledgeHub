...
const fetchGithubStars = async () => {
  try {
    ...
    // 公共API，可以直接使用fetch
    const response = await fetch('https://api.github.com/repos/pyhgoshift/AI-KnowledgeHub')
...
#app-router-view {
  ...
  overflow-y: auto;
  background:
    linear-gradient(90deg, rgba(189, 187, 255, 0.16) 0, rgba(189, 187, 255, 0) 180px),
    var(--research-paper);
}
...
.header {
  ...
  height: 100%;
  width: @sidebar-width;
  border-right: 1px solid var(--research-hairline);
  ...
  .sidebar-toggle {
    ...
    &:hover,
    &:focus-visible {
      border-color: var(--research-hairline);
      background: var(--research-paper);
      color: var(--research-ink);
      outline: none;
    }
  ...
  .nav-item {
    ...
    &.active {
      ...
      box-shadow: inset 3px 0 0 var(--research-periwinkle);
      font-weight: 500;
      color: var(--research-ink);
    }
    ...
    &.github {
      ...
      .github-stars {
        ...
        font-size: 12px;
        color: var(--gray-600);
        background-color: var(--research-sky);
        padding: 2px 8px;
        border-radius: var(--radius-ui);
        white-space: nowrap;
        transition:
