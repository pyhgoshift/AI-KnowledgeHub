      ...
      >
        <div class="conversation-search-input-row">
          <input
            ...
            aria-label="대화 검색"
          />
          <button type="button" class="conversation-search-close" aria-label="닫기" @click="close">
            <X :size="20" />
          </button>
        </div>
            ...
            <div v-else-if="results.length > 0" class="conversation-search-results">
              ...
              >
                ...
                <span class="result-main">
                  <span class="result-title">{{ item.title || '새 대화' }}</span>
              ...
              <div v-if="isLoadingMore" class="conversation-search-loading-more">불러오는 중...</div>
            </div>

            <div v-else class="conversation-search-empty">관련 대화를 찾지 못했습니다</div>
          </template>
          ...
          <template v-else>
            ...
            <template v-for="row in recentRows" :key="row.key">
              ...
              >
                ...
                <span>{{ row.thread.title || '새 대화' }}</span>
              </button>
            </template>
...
const formatResultDate = (value) => {
  ...
  if (!parsed) return ''
  if (parsed.year() === dayjs().year()) return parsed.format('M월 D일')
