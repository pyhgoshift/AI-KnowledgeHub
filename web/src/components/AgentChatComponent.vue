    ...
    >
      <div class="chat-header" :class="{ 'has-active-thread': !!currentChatId }">
        <div class="header__left">
          ...
          <div
            v-if="currentThread?.title && currentThread.title !== '새 대화'"
          ...
          <div class="bottom" :class="{ 'start-screen': !conversations.length }">
            ...
            <div class="message-input-wrapper">
              ...
              <div class="bottom-actions" v-if="conversations.length > 0">
                <p class="note">현재 에이전트: {{ currentThreadAgentName }} · 답변의 정확성을 확인하세요.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div v-if="statePanelOpen" class="state-panel">
            <div class="side-panel__header state-panel-header">
              <span class="state-panel-title">상태</span>
            ...
            <div class="state-panel-body">
              ...
              >
                ...
                >
                  <span class="state-section-label">
                    <span class="state-section-title">컨텍스트 사용량</span>
                    <ChevronDown
                      :size="15"
                ...
                >
                  <div class="token-usage-content">
                    <div class="token-usage-stack">
                      ...
                      <div class="token-usage-stack-track" aria-label="토큰 구성">
                        <div
              ...
              >
                ...
                >
                  <span class="state-section-label">
                    <span class="state-section-title">할 일</span>
                    <ChevronDown
                      :size="15"
                      class="state-section-chevron"
                      :class="{ 'is-collapsed': !isStateSectionExpanded('todos') }"
              ...
              >
                ...
                >
                  <span class="state-section-label">
                    <span class="state-section-title">첨부 파일</span>
                    <ChevronDown
                      :size="15"
                      class="state-section-chevron"
                      :class="{ 'is-collapsed': !isStateSectionExpanded('files') }"
              ...
              >
                ...
                >
                  <span class="state-section-label">
                    <span class="state-section-title">결과물</span>
                    <ChevronDown
                      :size="15"
                      class="state-section-chevron"
                      :class="{ 'is-collapsed': !isStateSectionExpanded('artifacts') }"
              ...
              >
                ...
                >
                  <span class="state-section-label">
                    <span class="state-section-title">하위 에이전트</span>
                    <ChevronDown
                      :size="15"
                      class="state-section-chevron"
                      :class="{ 'is-collapsed': !isStateSectionExpanded('subagents') }"
                  ...
                  <div class="state-list">
                    ...
                    >
                      <FallbackAvatar
                        ...
                        :size="28"
                        shape="rounded"
                        :alt="`${getSubagentRunName(run)} 아이콘`"
                      />
                      <div class="state-list-item-body">
              ...
              <div v-if="!hasVisibleStateSections" class="state-panel-empty">표시할 상태 정보가 없습니다</div>
            </div>
          </div>
        </div>
      </div>
...
const getSubagentRunName = (run) => {
  ...
  return (
    run?.subagent_name || currentSubagentOptionBySlug.value.get(subagentSlug)?.name || '하위 에이전트'
  )
}
...
const getSubagentRunMeta = (run) => {
  ...
  return artifacts ? `결과물 ${artifacts}개` : run?.id || ''
}
...
const tokenUsageSegments = computed(() => {
  ...
  const rawSegments = [
    ...
    {
      key: 'tools',
      label: `도구 정의 (${usage.tool_count || 0})`,
      value: toolsTokens,
      tone: 'is-tools'
    }
  ...
  return rawSegments.map((segment) => {
    ...
    return {
      ...
      valueLabel: segment.messageCount
        ? `${formatTokenCount(segment.value)} (메시지 ${segment.messageCount}개)`
...
const currentStateFiles = computed(() => {
  ...
  const pushFile = (entry, fallbackName = '파일') => {
  ...
  currentThreadAttachments.value.forEach((attachment) => pushFile(attachment, '첨부 파일'))
...
const replyLoadingText = computed(() =>
  currentThreadState.value?.contextCompressing ? '컨텍스트를 압축하는 중...' : '답변을 생성하는 중...'
)
...
const ensureActiveThread = async (title = '새 대화') => {
  if (currentChatId.value) return currentChatId.value
  try {
    const newThread = await createThread(currentAgentId.value, title || '새 대화')
    if (newThread) {
...
const ensureAttachmentThread = async () => {
  if (currentChatId.value) return currentChatId.value
  return await ensureActiveThread('새 대화')
}
...
const handleSendMessage = async ({ image } = {}) => {
  ...
  if (!threadId) {
    ...
    if (!threadId) {
      message.error('대화를 만들지 못했습니다. 다시 시도하세요')
      return
    }
  ...
  try {
    ...
    if (!runId) {
      throw new Error('실행을 만들지 못했습니다: run_id가 없습니다')
    }
...
const handleApprovalWithStream = async (answer) => {
  ...
  const threadState = getThreadState(threadId)
  if (!threadState) {
    message.error('해당 대화 스레드를 찾지 못했습니다')
    approvalState.showModal = false
    return
  }

  if (!interruptedRunId) {
    message.error('재개할 실행 작업을 찾지 못했습니다')
    approvalState.showModal = false
    return
  }
  ...
  try {
    ...
    const runId = runResp?.run_id
    if (!runId) {
      throw new Error('재개 실행을 만들지 못했습니다: run_id가 없습니다')
    }
...
const buildExportPayload = () => {
  ...
  const payload = {
    chatTitle: currentThread.value?.title || '새 대화',
    agentName: currentAgentName.value || currentAgent.value?.name || 'AI 어시스턴트',
